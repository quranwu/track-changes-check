...
metaData=function(MicrobData,CovData,linkIDname,testCov=NULL,ctrlCov=NULL,
  ...
  if(is.matrix(MicrobData))MdataWithId=cbind(MicrobData[,linkIDname,drop=FALSE],data.matrix(MicrobData[,!colnames(MicrobData)%in%linkIDname]))
  if(is.data.frame(MicrobData))MdataWithId=cbind(MicrobData[,linkIDname,drop=FALSE],data.matrix(MicrobData[,!colnames(MicrobData)%in%linkIDname]))
  if(is.character(MicrobData)){
    nCharac=nchar(MicrobData)
    if(substr(MicrobData,(nCharac-2),nCharac)=="csv"){
      MdataWithId=read.csv(file=MicrobData,header=TRUE,na.strings=c("","NA"))
    }
    if(substr(MicrobData,(nCharac-2),nCharac)=="tsv"){
      MdataWithId=read.table(file=MicrobData, sep='\t',header=TRUE,na.strings=c("","NA"))
    }
  }
  ...
  # read covariate data
  if(is.matrix(CovData))CovarWithId=cbind(CovData[,linkIDname,drop=FALSE],data.matrix(CovData[,!colnames(CovData)%in%linkIDname]))
  if(is.data.frame(CovData))CovarWithId=cbind(CovData[,linkIDname,drop=FALSE],data.matrix(CovData[,!colnames(CovData)%in%linkIDname]))
  if(is.character(CovData)){
    nCharac=nchar(CovData)
    if(substr(CovData,(nCharac-2),nCharac)=="csv"){
      CovarWithId=read.csv(file=CovData,header=TRUE,na.strings=c("","NA"))
    }
    if(substr(CovData,(nCharac-2),nCharac)=="tsv"){
      CovarWithId=read.table(file=CovData, sep='\t',header=TRUE,na.strings=c("","NA"))
    }
  }
  ...
  Covariates1=data.matrix(CovarWithId[,!colnames(CovarWithId)%in%linkIDname,drop=FALSE])
  ...
  allRawData=na.omit(merge(CovarWithId1,MdataWithId,by=linkIDname,all.x=FALSE,all.y=FALSE))

  CovarWithId=allRawData[,(colnames(allRawData)%in%colnames(CovarWithId1)),drop=FALSE]

  Covariates=data.matrix(CovarWithId[,!colnames(CovarWithId)%in%linkIDname,drop=FALSE])
  rm(CovarWithId1)
  ...
  if(numTaxaNoReads==0){results$data=dataOmit}
  if(numTaxaNoReads>0){
